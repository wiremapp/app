...
export const PricingComponent = (props) => {
  ...
  return (
    ...
    >
      <div>
        <div className="row">
          <h3>{props.locale.t("pricing_label")}</h3>
          <div className="flex items-center justify-center lg:mb-12">
            ...
            >
              {props.locale.t("billed_annually_label")}
            </label>
          </div>

          <div className="mb-6 grid w-full grid-cols-1 gap-4  space-x-2 lg:grid-cols-3">
            {sortedData.map((tier) => {
              const tierTitle = tier.frontMatter.title;
              const monthlyPrice =
                locale === "GB"
                  ? tier.frontMatter.monthly_price_gbp
                  : tier.frontMatter.monthly_price_usd;
              const annualPrice =
                locale === "GB"
                  ? tier.frontMatter.annual_price_gbp
                  : tier.frontMatter.annual_price_usd;

              const displayPrice = !annual ? monthlyPrice : annualPrice;
              ...
              return (
                <div
                  key={tier.slug}
                  className={`rounded p-8 hover:bg-[#03030328] ${
                    !isLg && "section-secondary-bg"
                  } transition-all ${
                    tier.frontMatter.primary
                      ? "section-secondary-bg py-4 shadow"
                      : "my-[48px]"
                  }`}
                >
                  <div>
                    <div className="flex flex-grow flex-col items-center lg:items-stretch">
                      ...
                      <ul
                        role="list"
                        className="mb-8 flex h-80 w-full flex-col justify-center space-y-4 lg:justify-normal"
                      >
                    ...
                    {tier.frontMatter.primary ? (
                      <>
                        <hr className="border-0 lg:pb-[110px]" />
                      </>
                    ) : null}
                    <div className="flex flex-col">
                      <Button
                        ...
                        space={"medium"}
                      >
                        {props.locale.t("getStarted_label")}
                      </Button>
                    </div>
                  </div>
                </div>
